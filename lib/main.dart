...
void main() {
  ...
  while (!game.state.isGameOver && roundCount < 10) {
    ...
    print('Player 1 selected cards: ${p1.selectedCards.map((c) => c.name).join(", ")}');
    ...
    print('Player 2 selected cards: ${p2.selectedCards.map((c) => c.name).join(", ")}');
...
void _displayPlayerInfo(PlayerState player) {
  ...
  if (player.hand.isNotEmpty) {
    print('  Cards in hand: ${player.hand.map((c) => '${c.name}(${c.value})').join(", ")}');
  }
}